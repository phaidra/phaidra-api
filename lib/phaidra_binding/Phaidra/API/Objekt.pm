...
#	my $res = $soap->ingest(SOAP::Data->type(base64 => encode("utf-8", $foxml)), 'info:fedora/fedora-system:FOXML-1.1', 'Ingested by Phaidra API');
    ...
    if ( $res->fault ) {
        $log->logdie(
            "Ingest failed: " . $res->faultcode . ": " . $res->faultstring );
    }

    $self->{PID} = $res->result;

    # set Content-Model
    $self->addRelationship( "info:fedora/fedora-system:def/model#hasModel",
        $contentmodel );

    # set OAI link
    $self->addRelationship( "http://www.openarchives.org/OAI/2.0/itemID",
            "oai:"
          . $self->{phaidra}->{config}->{proaiRepositoryIdentifier} . ":"
          . $self->{PID} );
    $self->{existing}       = 0;
    $self->{changed_rights} = 0;
...
=cut
    ...
    if ( $res->fault ) {
        $log->logdie(
            "load failed: " . $res->faultcode . ": " . $res->faultstring );
    }
    ...
    if ( defined($contentmodel) ) {
        ...
        unless ($valid) {
            $log->logdie( "Object is no $contentmodel object, can't load: "
                  . Dumper( $objectProfile->{'objModels'}->{'model'} ) );
        }
    }
    else {
        # auch ok
  ...
  # All objects can have a RIGHTS-DS. Load it and put it into internal structure
    ...
    eval {
        my $mimetype;
        ( $mimetype, $rightsxml ) =
          $self->getDissemination( "bdef:Asset", "getRights" );
    };
    if ($@) {
    ...
    if ( defined($rightsxml) ) {
        ...
        $xp->set_namespace( 'uwr',
            'http://phaidra.univie.ac.at/XML/V1.0/rights' );
...
sub addMetadata {
    ...
    $self->addDatastreamContent( "UWMETADATA", "text/xml", $metadata,
        $self->{phaidra}->{config}->{uwmetadatalabel}, "X" );
}
...
sub modifyMetadata {
    ...
    $self->modifyDatastreamByValue( "UWMETADATA", "text/xml", $metadata,
        $self->{phaidra}->{config}->{uwmetadatalabel}, "X" );
}
...
sub addDatastream {
    ...
    my $cred = encode_base64(
        $self->{phaidra}->{username} . ':' . $self->{phaidra}->{password} );
    ...
    $self->addDatastreamLocation( $dsid, $mimetype, $location, $label,
        $controlGroup );
}
...
sub addDatastreamLocation {
    ...
    push @args,
      SOAP::Data->new( name => 'controlGroup', value => $controlGroup );
    ...
    if ( $res->fault ) {
        $log->logdie( "addDatastream failed: "
              . $res->faultcode . ": "
              . $res->faultstring );
    }
...
sub purgeRelationship {
    ...
    push @args,
      SOAP::Data->new( name => 'object', value => "info:fedora/$object" );
    push @args,
      SOAP::Data->new(
        name  => 'isLiteral',
...
=cut
    ...
    if ( $res->fault ) {
        $log->logdie( "purgeRelationship failed: "
              . $res->faultcode . ": "
              . $res->faultstring );
    }
...
sub addRelationship {
    ...
    if ( $res->fault ) {
        $log->logdie( "addRelationship failed: "
              . $res->faultcode . ": "
              . $res->faultstring );
    }
...
sub addRelationships {
        ...
        push @rels,
          SOAP::Data->type("RelationshipTuple")->name(
            "relationships" => \SOAP::Data->value(
                ...
                SOAP::Data->name("object")
                  ->value( "info:fedora/" . $r->{object} ),
    ...
    if ( $res->fault ) {
        $log->logdie( "addRelationships failed: "
              . $res->faultcode . ": "
              . $res->faultstring );
    }
...
sub getRelationships {
    ...
    push @args, SOAP::Data->new( name => 'pid', value => $self->{PID} );
    push @args,
      SOAP::Data->new( name => 'relationship', value => $predicate ) ;
    ...
    if ( $res->fault ) {
        $log->logdie( "getRelationships failed: "
              . $res->faultcode . ": "
              . $res->faultstring );
    }
...
sub purgeRelationships {
    ...
    foreach my $r (@$relationships) {
        push @rels,
          SOAP::Data->type("RelationshipTuple")->name(
            "relationships" => \SOAP::Data->value(
                SOAP::Data->name("subject")->value( $self->{PID} ),
                SOAP::Data->name("predicate")
                  ->value( SOAP::Data->type( string => $r->{predicate} ) ),
                SOAP::Data->name("object")
                  ->value( "info:fedora/" . $r->{object} )->type("xsd:string"),
                SOAP::Data->name("isLiteral")->value(0)->type("boolean"),
                SOAP::Data->name("datatype")->value(undef)
            )
          );
    ...
    if ( $res->fault ) {
        $log->logdie( "purgeRelationships failed: "
              . $res->faultcode . ": "
              . $res->faultstring );
    }
...
sub modifyDatastreamByValue {
    ...
    if ( $res->fault ) {
        $log->logdie( "modifyDatastreamByValue failed: "
              . $res->faultcode . ": "
              . $res->faultstring );
    }
...
sub modifyDatastreamByReference {
    ...
    if ( $res->fault ) {
        $log->logdie( "modifyDatastreamByReference failed: "
              . $res->faultcode . ": "
              . $res->faultstring );
    }
...
sub getDatastream {
    ...
    if ( $res->fault ) {
        $log->logdie( "getDatastreamDissemination failed: "
              . $res->faultcode . ": "
              . $res->faultstring );
    }
    my $dsi = $res->result;
...
sub getDissemination {
    ...
    push @args, SOAP::Data->new( name => 'pid', value => $self->{PID} );
    push @args,
      SOAP::Data->new( name => 'serviceDefinitionPid', value => $sdef );
    ...
    if ( $res->fault ) {
        $log->logdie( "getDissemination failed: "
              . $res->faultcode . ": "
              . $res->faultstring );
    }
    my $dsi = $res->result;
...
sub save {
    ...
    push @args, SOAP::Data->new( name => 'pid', value => $self->{PID} );
    push @args,
      SOAP::Data->new( name => 'state', value => 'A' );
        push @args,
      SOAP::Data->new(
        name  => 'logMessage',
       ...
       ###     'Changed by Phaidra API' );
        ...
        if ( $res->fault ) {
            $log->logdie( "modifyObject (A) failed: "
                  . $res->faultcode . ": "
                  . $res->faultstring );
        }
...
# So create a new one and add/update
    if ( $self->{changed_rights} ) {
        ...
        my $root   = $rights->createElementNS(
            "http://phaidra.univie.ac.at/XML/V1.0/rights", "uwr:rights" );
        ...
        foreach my $s ( keys %{ $self->{rights}->{usernames} } ) {
            ...
            if ( defined( $self->{rights}->{usernames}->{$s}->{expires} ) ) {
                $node->setAttribute( "expires",
                    $self->{rights}->{usernames}->{$s}->{expires} );
            }
        ...
        foreach my $s ( keys %{ $self->{rights}->{faculties} } ) {
            ...
            if ( defined( $self->{rights}->{faculties}->{$s}->{expires} ) ) {
                $node->setAttribute( "expires",
                    $self->{rights}->{faculties}->{$s}->{expires} );
            }
            $allow->appendChild($node);
        }
        foreach my $s ( keys %{ $self->{rights}->{departments} } ) {
            ...
            if ( defined( $self->{rights}->{departments}->{$s}->{expires} ) ) {
                $node->setAttribute( "expires",
                    $self->{rights}->{departments}->{$s}->{expires} );
            }
            $allow->appendChild($node);
        }
        ...
        foreach my $s ( keys %{ $self->{rights}->{kennzahl} } ) {
            ...
            if ( defined( $self->{rights}->{kennzahl}->{$s}->{expires} ) ) {
                $node->setAttribute( "expires",
                    $self->{rights}->{kennzahl}->{$s}->{expires} );
            }
            $allow->appendChild($node);
        }
...
# Also an existing object may not have a RIGHTS-DS - so try modifyDatastream and then addDatastream
        eval {
            $self->modifyDatastreamByValue( "RIGHTS", "text/xml",
                $rights->toString(), "Rights", "X" );
        };
        if ($@) {
            $self->addDatastreamContent( "RIGHTS", "text/xml",
                $rights->toString(), "Rights", "X" );
        }
        $self->{changed_rights} = 0;
    }
...
sub revoke {
    ...
    $log->debug(
        "revoke $what $who: rights now: " . Dumper( $self->{rights} ) );
}
...
sub addObjectToPapers {
    ...
    foreach my $pid (@$pids) {
        push @relationships,
          {
            predicate =>
              "info:fedora/fedora-system:def/relations-external#isPartOfPaper",
            object => $pid
          };
    }
...
sub modifyObject {
    ...
    if ( $res->fault ) {
        $log->logdie( "modifyObject failed: "
              . $res->faultcode . ": "
              . $res->faultstring );
    }
